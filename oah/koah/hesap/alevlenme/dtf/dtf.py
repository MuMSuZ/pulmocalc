...
@app.route('/')
def hesaplama():
    return render_template('koah.html')

@app.route('/dtf', methods=['GET', 'POST'])  # POST yöntemini ekledik
def nivsure():
    if request.method == 'POST':  # POST isteği kontrolü
        try:
            ...
            if ekspiryum_kalinligi == 0:
                return redirect(url_for('dtf', result="Ekspiryum kalınlığı 0 olamaz."))
            ...
            return redirect(url_for('dtf', result=result))
        except ValueError:
            return redirect(url_for('dtf', result="Lütfen geçerli sayılar giriniz."))
    result = request.args.get('result')  # Sonucu GET parametresinden al
    return render_template('dtf.html', result=result)
